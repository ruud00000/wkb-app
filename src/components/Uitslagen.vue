...
<script setup lang="ts" target="_blank" rel="noopener">
const urls = Object.entries(import.meta.glob('/src/assets/*.pdf', { eager: true, as: 'url' })).reduce((acc, [key, value]) => {
  acc.push([key.replace('/src/assets/', ''), value])
  return acc
